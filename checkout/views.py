...
def get_booking_recipient_email(booking):
    ...
    verified_email = EmailAddress.objects.filter(
        user=booking.user, verified=True
    ).first()
    if verified_email:
...
def send_booking_confirmation_email(booking):
    ...
    if (
        ...
        logger.warning(
            "EMAIL_BACKEND is console; confirmation email is "
            "printed to terminal and not delivered to inbox."
        )

    recipient = get_booking_recipient_email(booking)
    if not recipient:
        logger.warning(
            f"No recipient email available for booking {booking.booking_id}"
        )
        return
    ...
    try:
        ...
        logger.info(f"Booking confirmation email sent for {
            booking.booking_id} to {recipient}")
    except Exception as exc:
        logger.error(f"Failed to send booking confirmation email for {
...
def checkout(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(
                request, "Booking not found. Please contact support."
            )
    ...
    # Apply delivery threshold logic if applicable (set to 0 for class
    # bookings)
    delivery = 0
    ...
    try:
        ...
        stripe.api_key = settings.STRIPE_SECRET_KEY
        logger.debug(
            f"Stripe API Key before Payment Intent: {stripe.api_key[:20]}..."
        )
        ...
        logger.error(f"Stripe API Error - Type: {
...
def cache_checkout_data(request):
    ...
    try:
        ...
        if not cart_items:
            return JsonResponse(
                {"error": "No valid items in cart"}, status=400
            )

        first_class_item = next(
            (item for item in cart_items if item["item_type"] == "class"), None
        )
        course = (
            first_class_item["exercise_class"] if first_class_item else None
        )

        # For package-only carts, capture the instructor from the first package
        # item
        first_package_item = next(
            (item for item in cart_items if item["item_type"] == "package"),
            None,
        )
        ...
        return JsonResponse(
            {"success": True, "booking_id": str(booking.booking_id)}
        )
    except Exception as e:
        logger.error(
            f"Error in cache_checkout_data: {type(e).__name__}: {str(e)}",
            exc_info=True,
        )
        messages.error(
            request,
...
def checkout_success(request, booking_id):
    ...
    messages.success(
        request, f"Booking confirmed! Your booking ID is {booking_id}"
    )
...
def stripe_webhook(request):
    ...
    try:
        ...
        logger.error(
            f"Stripe webhook construct_event failed: {type(e).__name__}: {e}"
        )
        return HttpResponse(status=400)
...
def handle_payment_intent_succeeded(payment_intent):
    ...
    try:
        ...
        logger.error(f"Error updating booking for payment intent {pid}: {
