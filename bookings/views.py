...
def my_bookings(request):
    ...
    bookings = ClassBooking.objects.filter(user=request.user).select_related('course', 'course__instructor').order_by('-created_at')
    
    # Separate into upcoming and past bookings
    now = timezone.now()
    upcoming_bookings = bookings.filter(course__start_datetime__gte=now)
...
def cancel_booking(request, booking_id):
    ...
    if booking.course.start_datetime <= timezone.now():
