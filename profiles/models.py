...
class Instructor(models.Model):
    ...
    lesson_description = models.TextField(
        blank=True,
        help_text=(
            "Description of lesson style, approach, and "
    ...
    specialties = models.CharField(
        max_length=500,
        blank=True,
        help_text="Comma-separated specialties (e.g., Yoga, Pilates, HIIT)",
    )
    class_type = models.CharField(
        max_length=100,
        blank=True,
        help_text=(
            "Primary class type (e.g., Personal Trainer, Yoga, "
            "Pilates, Boxercise)"
        ),
    )
    ...
    image = models.ImageField(
        null=True, blank=True, help_text="Professional photo"
    )
    ...
    def __str__(self):
        return (
            f"{self.user.get_full_name() or self.user.username} - Instructor"
        )
