...
class InstructorAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        category_choices = [('', '---------')] + [
            (category.name, category.name) for category in Category.objects.order_by('name')
        ]

        current_value = self.instance.class_type if self.instance and self.instance.pk else ''
        if current_value and current_value not in {value for value, _label in category_choices}:
