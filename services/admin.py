...
class ExerciseClassAdmin(admin.ModelAdmin):
    ...
    def duplicate_selected_classes(self, request, queryset):
        ...
        for exercise_class in queryset:
            ...
            exercise_class.start_datetime = exercise_class.start_datetime + timedelta(days=7)
            exercise_class.end_datetime = exercise_class.end_datetime + timedelta(days=7)
            exercise_class.save()
            duplicated_count += 1

        self.message_user(
            request,
            f'{duplicated_count} class(es) duplicated with schedule shifted by 7 days.'
        )
